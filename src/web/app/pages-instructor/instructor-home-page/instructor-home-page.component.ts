...
import {
  Index,
  MutateEvent,
} from "../../components/sessions-table/sessions-table.component";
  ...
  implements OnInit
{
        ...
        this.courseService
          ...
          .subscribe({
            next: (courseArchive: CourseArchive) => {
              ...
              this.statusMessageService.showSuccessToast(
                `The course ${courseArchive.courseId} has been archived. ` +
   ...
   */
  sortSessionsTableRowModelsEvent(
    tabIndex: number,
    event: SortableEvent
  ): void {
    ...
    tab.sessionsTableRowModels.sort(
      this.sortModelsBy(event.sortBy, event.sortOrder)
    );
  }
  ...
  publishSessionEventHandler(tabIndex: number, event: MutateEvent): void {
    const model =
      this.courseTabModels[tabIndex].sessionsTableRowModels[event.idx];
