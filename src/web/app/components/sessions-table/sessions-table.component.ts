...
export class SessionsTableComponent implements OnInit {
  ...
  @Output()
  sortSessionsTableRowModelsEvent: EventEmitter<SortableEvent> =
    new EventEmitter();
  ...
  @Output()
  sendRemindersToAllNonSubmittersEvent: EventEmitter<Index> =
    new EventEmitter();

  @Output()
  sendRemindersToSelectedNonSubmittersEvent: EventEmitter<Index> =
    new EventEmitter();
  ...
  createColumnData(config: SessionsTableColumnData): ColumnData[] {
    console.log("Creating column:", config.header);
    console.log("columnsToShow:", this.columnsToShow);
    console.log("config.columnType:", config.columnType);
    if (
      //columnType = not undefined
      !(config?.columnType === undefined) &&
      !this.columnsToShow.includes(config.columnType!)
    ) {
      return [];
    }
    //columnType = undefined and
    console.log("Creating column:", config.header);
  ...
  setRowData(): void {
    this.rowsData = this.sessionsTableRowModelsVar.map(
      (sessionTableRowModel: SessionsTableRowModel) => {
        ...
        return [
          ...
          ...this.createRowData({
            columnType: SessionsTableColumn.START_DATE,
            ...this.createDateCellWithToolTip(
              submissionStartTimestamp,
              timeZone
            ),
          }),
          ...
          ...this.createRowData(
            this.createCellWithToolTip(
              this.submissionStatusTooltip.transform(
                submissionStatus,
                deadlines
              ),
          ...
          ...this.createRowData(
            this.createCellWithResponseRateComponent(sessionTableRowModel)
          ),
          ...this.createRowData(
            this.createCellWithGroupButtonsComponent(sessionTableRowModel)
          ),
        ];
      }
    );
  ...
  private createCellWithGroupButtonsComponent(
    ...
    const { courseId, feedbackSessionName, submissionStatus, publishStatus } =
      feedbackSession;
      ...
      customComponent: {
        ...
        componentData: (idx: number) => {
          return {
            ...
            publishSession: () =>
              this.publishSession(idx, this.rowsData[idx], this.columnsData),
            unpublishSession: () =>
              this.unpublishSession(idx, this.rowsData[idx], this.columnsData),
  ...
  private createDateCellWithToolTip(
    timestamp: number,
    timeZone: string
  ): SortableTableCellData {
    return {
   ...
   */
  sortSessionsTableRowModelsEventHandler(event: {
    sortBy: SortBy;
    sortOrder: SortOrder;
  }): void {
  ...
  moveSessionToRecycleBin(idx: number): void {
    ...
    modalRef.result.then(
      () => {
        ...
        this.sessionsTableRowModelsVar = this.sessionsTableRowModelsVar.filter(
          (_, index) => index !== idx
        );
        this.setRowData();
      },
  ...
  copySession(rowIndex: number): void {
    ...
    const model: SessionsTableRowModel =
      this.sessionsTableRowModelsVar[rowIndex];
  ...
  unpublishSession(
    ...
    modalRef.result.then(
      () => {
        this.unpublishSessionEvent.emit({
          idx: rowIndex,
          rowData,
          columnsData,
        });
